...
function getNextPrevBooks(bookId) {
    ...
    let prevBookIndex = currBookIndex - 1
    let nextBookIndex = currBookIndex + 1
    if (currBookIndex === 0) prevBookIndex = gBooks.length - 1
    else if (currBookIndex === gBooks.length - 1) nextBookIndex = 0
    const prevBook = gBooks[prevBookIndex]
    const nextBook = gBooks[nextBookIndex]
...
function _createbooks() {
    if (!gBooks) gBooks = storageService.load(BOOKS_KEY);
    if (!gBooks)
        gBooks = [{
            "id": "OXeMG8wNskc",
